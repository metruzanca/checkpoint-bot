...
var GoalCmd = &Command{
	...
	Handler: func(db database.CheckpointDatabase, s *discordgo.Session, i *discordgo.InteractionCreate) {
		...
		log.Info("goal modal opened", "checkpoint_id", checkpoint.ID, "user", targetUserID, "is_admin_override", isAdminOverride, "has_existing_goal", err == nil)
	},
}
...
func HandleGoalModalSubmission(db database.CheckpointDatabase, s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	// Parse custom ID: goal_modal_{checkpoint_id}_{user_id}
	parts := strings.Split(data.CustomID, "_")
	if len(parts) != 4 || parts[0] != "goal" || parts[1] != "modal" {
	...
	s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
		Type: discordgo.InteractionResponseChannelMessageWithSource,
		Data: &discordgo.InteractionResponseData{
			Content: fmt.Sprintf("Goal %s successfully!", action),
			Flags:   discordgo.MessageFlagsEphemeral,
		},
	})
}
